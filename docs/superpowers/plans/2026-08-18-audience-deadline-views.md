...
- [ ] Run `streamlit run views/sales_view.py --server.port 8503`, confirm it shows only Team/Away Team/Match Date/Sales Deadline for real upcoming fixtures, with no way to navigate to the Delivery app or Admin page from it. (Note: an earlier version of this plan had these scripts at the repo root, which a final review found let Streamlit's multipage auto-discovery attach the Archive/Admin pages to their sidebar, defeating the access separation — fixed by moving both scripts into `views/`, a directory with no `pages/` sibling.)
- [ ] Run `streamlit run views/partner_success_view.py --server.port 8504`, confirm it shows the platform list correctly for a real team with configured platforms.
